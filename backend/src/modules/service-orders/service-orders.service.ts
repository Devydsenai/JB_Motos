...
export class ServiceOrdersService {
  ...
  async create(dto: CreateServiceOrderDTO, atendenteId?: string) {
    const origem = dto.origem ?? "OFICINA";

    if (origem === "OFICINA") {
      if (!dto.motocicletaId) throw new AppError("motocicletaId é obrigatório para OS da oficina", 422);
      if (!atendenteId) throw new AppError("Atendente é obrigatório", 422);
      ...
      return this.persistOrder(dto, {
        ...
        atendenteId,
        historyUserId: atendenteId,
      });
    }

    // LOJA_ONLINE
    if (dto.storeCustomerId) {
      const customer = await prisma.storeCustomer.findUnique({
        where: { id: dto.storeCustomerId },
      });
      if (!customer) throw new NotFoundError("Cliente da loja");
    }

    if (dto.productIds?.length) {
      const count = await prisma.product.count({
        where: { id: { in: dto.productIds }, ativo: true },
      });
      if (count !== dto.productIds.length) {
        throw new AppError("Um ou mais produtos são inválidos", 422);
      }
    }

    return this.persistOrder(dto, {
      origem: "LOJA_ONLINE",
      storeCustomerId: dto.storeCustomerId,
      atendenteId,
      historyUserId: atendenteId,
  ...
  private async persistOrder(
    ...
    return prisma.$transaction(async (tx) => {
      const os = await tx.serviceOrder.create({
        data: {
          origem: ctx.origem,
          motocicletaId: ctx.motocicletaId,
          storeCustomerId: ctx.storeCustomerId,
          ...
          ...(ctx.productIds?.length && {
            products: {
              create: await Promise.all(
                ctx.productIds.map(async (productId) => {
        ...
        include: {
          ...
          storeCustomer: { select: { id: true, nome: true, email: true } },
          atendente: { select: { id: true, nome: true } },
          products: { include: { product: true } },
        },
      });

      if (ctx.historyUserId) {
        await tx.oSHistory.create({
          data: {
            serviceOrderId: os.id,
            userId: ctx.historyUserId,
            statusAnterior: null,
            statusNovo: "ABERTA",
            observacao:
              ctx.origem === "LOJA_ONLINE"
                ? "Solicitação recebida pela loja online"
