...
export const findById = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ok(res, await clientsService.findById(param(req.params.id)));
  } catch (err) { next(err); }
};
...
export const update = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ok(res, await clientsService.update(param(req.params.id), req.body), "Cliente atualizado");
  } catch (err) { next(err); }
};

export const remove = async (req: Request, res: Response, next: NextFunction) => {
  try {
    await clientsService.softDelete(param(req.params.id));
    noContent(res);
  } catch (err) { next(err); }
};
...
export const history = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ok(res, await clientsService.getHistory(param(req.params.id)));
  } catch (err) { next(err); }
};
